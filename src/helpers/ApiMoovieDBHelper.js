...
import { API_KEY_MOVIE_DB, URL_POSTER_PATH_MOVIE, URL_API_MOVIE_V3 } from "./constants";
...
const getRamdomActor = async () => {
  ...
  const actors = await fetch(
    `${URL_API_MOVIE_V3}person/popular?api_key=${API_KEY_MOVIE_DB}&language=en-US&page=1`,
    myInit
  ).then((res) => res.json());
...
const getRandomMovieDetails = async () => {
  ...
  const moviesList = await fetch(
    `
    ${URL_API_MOVIE_V3}list/1?api_key=${API_KEY_MOVIE_DB}&language=en-US`,
    myInit
  ).then((res) => res.json());
  ...
  const movieCredit = await fetch(
    `
    ${URL_API_MOVIE_V3}movie/${randomMovie.id}/credits?api_key=${API_KEY_MOVIE_DB}&language=en-US`,
    myInit
  ).then((res) => res.json());
